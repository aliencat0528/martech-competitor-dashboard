...
**圖說走同一條路**：圖表的 `summary` 由 L2 用句型模板＋算出的數字組出來，
與圖用的是同一組數字，所以圖改了描述一定跟著改；描述裡的主張句（`takeaway`）
同樣帶 `evidence_ids`。呈現層若自己寫一段文字描述圖，那段文字就會在下次更新資料時
默默過期——這是本專案最容易發生、也最難被發現的一種錯（規格見 `docs/FEATURES.md`「圖說」）。
...
4. **能力字典是否擴充零售側欄位** → 決定 `capabilities.json` 是走大字典還是分群子集，
   後者會讓 Zone C 需要支援「換群換欄位」

> 原第 1 項「落差分的評分規則」已隨 MC-009 撤銷 MC-007 而消滅，`src/analysis/gap.ts` 不再需要。
